...
public class FreeServiceImpl implements FreeService {
	...
	public String getNick(Free viewFree) {
		return freeDao.selectNickByUserId(JDBCTemplate.getConnection(), viewFree);
	}
	...
	public void write(HttpServletRequest req) {
		...
		if(free != null) {
			...
			free.setBoardNo(boardNo); //게시글 번호 입력 (PK)
		...
		//첨부파일 정보가 있을 경우
		if(freeFile != null) {
			freeFile.setBoardNo(boardNo); //게시글 번호 입력 (FK)
	...
	public void update(HttpServletRequest req) {
		...
		//첨부파일 정보가 있을 경우
		if(freeFile != null) {
			freeFile.setBoardNo(free.getBoardNo()); //게시글 번호 입력 (FK)
